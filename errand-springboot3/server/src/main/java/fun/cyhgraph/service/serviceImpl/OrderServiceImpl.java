...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderSubmitVO submit(OrderSubmitDTO orderSubmitDTO) {
        ...
        // 由于不走微信支付，直接设为"待接单"状态和"已支付"，后台才能看到订单
        order.setStatus(Order.TO_BE_CONFIRMED); // 待接单
        order.setPayStatus(Order.PAID); // 已支付
        order.setOrderTime(LocalDateTime.now());
        order.setCheckoutTime(LocalDateTime.now()); // 结账时间
        // 字段默认值：防止前端未传时 BeanUtils 复制 null 导致 SQL 违反 NOT NULL 约束
        if (order.getDeliveryStatus() == null) order.setDeliveryStatus(1);  // 立即送出
        if (order.getTablewareStatus() == null) order.setTablewareStatus(1); // 按餐量提供
        // 根据购物车中的商品计算订单总金额
...
//        }
    ...
    public OrderPaymentVO payment(OrderPaymentDTO orderPaymentDTO) {
        ...
        // 根据订单号从数据库查订单（不能用 this.order，因为支付是独立请求）
