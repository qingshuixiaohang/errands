...
public class UserServiceImpl implements UserService {
    ...
    public User wxLogin(UserLoginDTO userLoginDTO) {
        ...
        // 1. 先尝试调微信API获取openid（加 try-catch 防止网络异常导致整个登录失败）
        try {
            openid = getOpenId(code);
        } catch (Exception e) {
            System.out.println("【本地开发】调微信API异常: " + e.getMessage());
            openid = null;
        }

        // 2. 如果微信API返回失败（模拟器/本地环境），生成固定模拟openid
        //    注意：不能用时间戳，否则每次登录都会生成不同用户导致历史数据丢失
        if (openid == null) {
            openid = "mock_openid_dev";
            System.out.println("【本地开发】微信登录失败，使用固定模拟openid: " + openid);
        }
