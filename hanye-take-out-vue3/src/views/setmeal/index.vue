...
// 新增和修改套餐组合都是同一个页面，不过要根据路径传参的方式来区分
...
<template>
  <el-card>
    <div class="horizontal">
      <el-input size="large" class="input" v-model="pageData.name" placeholder="请输入组合名" />
      <el-select size="large" class="input" clearable v-model="pageData.categoryId" placeholder="选择分类">
        <el-option v-for="item in categoryList" :key="item.id" :label="item.name" :value="item.id" />
      </el-select>
      <el-select class="input" clearable v-model="pageData.status" placeholder="选择组合状态" size="large">
        <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value" />
      </el-select>
      <el-button size="large" class="btn" round type="success" @click="showPageList()">查询组合</el-button>
    ...
    <el-table ref="multiTableRef" :data="setmealList" stripe @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="name" label="组合名称" align="center" />
